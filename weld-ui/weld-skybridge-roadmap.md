...
- **Cross-tab coordination.** GM storage is shared across tabs/sessions; the companion can relay between two open generators — a GM-runs-the-world tab driving a player tab, or a director/stage split. Local "multiplayer," no server. *Edge:* it's near-real-time via polling, not instant; fine for turn-based.
